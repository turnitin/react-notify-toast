...
function renderToast(text, type, timeout, color) {
  const target = document.getElementById(getDefaults().wrapperId);
...
function show(text, type, timeout, color) {
  if (!document.getElementById(getDefaults().wrapperId).hasChildNodes()) {
    // Use default timeout if not set.
    const renderTimeout = timeout || getDefaults().timeout;
    ...
    setTimeout(() => {
      hide();
    }, renderTimeout + getDefaults().animationDuration);
...
function createShowQueue(initialRecallDelay = 500, recallDelayIncrement = 500) {
  ...
  this.showNotify = () => {
    ...
    if (show(current.text, current.type, current.timeout, current.color)) {
      ...
      if (current.timeout > 0) {
        setTimeout(() => this.showNotify(), current.timeout + getDefaults().animationDuration);
      }
    } else {
  ...
  return (text, type = '', timeout = getDefaults().timeout, color) => {
    this.msgs.push({
      text, type, timeout, color,
    });
    if (!this.isNotifying) {
      this.showNotify();
    }
